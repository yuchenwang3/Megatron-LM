    ...
    if wrap_with_ddp:
        ...
        with torch.cuda.stream(ddp_stream):
            ...
            if DP is DDP:
                ...
                if use_layer_wise_distributed_optimizer:
                    # LayerWise (Muon) optimizer. Force ddp_config.use_distributed_optimizer=True so
                    # sibling non-LayerWise buffers (embeddings, biases, layernorm) shard with the
                    # byte-level DistributedOptimizer layout, and tag params so DDP buffer grouping
                    # routes LayerWise-managed matrices to a separate buffer. The padded-vs-compact
                    # LayerWise layout decision is made inside compute_full_param_layout /
                    # _ParamAndGradBuffer from ddp_config.use_layer_wise_param_layout: the default
                    # padded layout keeps LayerWise buffers on DistOpt, while
                    # --no-use-layer-wise-param-layout selects the compact no-padding layout and the
                    # per-buffer override in _ParamAndGradBuffer turns use_distributed_optimizer back
                    # off for the Muon buffers.
                    ddp_config.use_distributed_optimizer = True
